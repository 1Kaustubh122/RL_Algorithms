...
from TD.expected_sara import expec_sara
...
def run_expec_sara(env, policy, num_episodes=1000, alpha=0.1, gamma=0.9):
    """
    Run TD prediction and return the value function as a grid.
    """
    expec_sara_instance = expec_sara(env=env, policy=policy, alpha=alpha, gamma=gamma)
    Q = expec_sara_instance.expec_sara(num_episode=num_episodes)
    grid = grid_from_q(Q, size=env.size)
    expec_sara_instance.print_q_policy()
    return grid
...
if __name__ == "__main__":
    ...
    val_grids["e-greedy policy"] = run_expec_sara(
    ...
    fig.suptitle("expec_sara State-Value Function")
    fig.tight_layout()
    plt.savefig("Results/expec_sara_Value_Function.png")
    plt.show()
