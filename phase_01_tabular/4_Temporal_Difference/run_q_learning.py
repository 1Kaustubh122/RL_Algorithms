...
from TD.Q_Learning import Q_Learning
...
def run_q_Learning(env, policy, num_episodes=1000, alpha=0.1, gamma=0.9):
    """
    Run TD prediction and return the value function as a grid.
    """
    q_lr_instance = Q_Learning(env=env, policy=policy, alpha=alpha, gamma=gamma)
    Q = q_lr_instance.q_learning(num_episode=num_episodes)
    grid = grid_from_q(Q, size=env.size)
    q_lr_instance.print_q_policy()
    return grid
...
if __name__ == "__main__":
    ...
    val_grids["e-greedy policy"] = run_q_Learning(
    ...
    fig.suptitle("Q_Learning State-Value Function")
    fig.tight_layout()
